...
# 좌표로 12 8 설명, 뺀 것과 동치다
# 44와 8이 있을 때 8 36 / 8 28 / 8 20 ... 되는데
# 결국 뺴기는 모듈러와 동치다
# 8 36, 8 28과 같은 과정을 생략하고 바로 8과 4로 넘어가겠다는 뜻


"""
# 최소 공배수
원본 둘을 곱한 다음에 최대공약수로 나누면 됨

2^3 3^2 5^2
2^2 3^6      7^3 있다 칠 때
최대공약수 : 2가 일단 2개 있어야 함 2^2 3^2 -> 작은 것만 고른 것, 5와 7은 없으니까 pass
최소공배수 : 큰 것만 고른 것. 2^3 3^6 5^2 7^3
이 둘을 곱하면 위 둘을 더한 것만 쭉 남게 됨
즉, 모두 더한 것들에서 최대공약수를 빼게 되면 큰 것만 남으므로
2^5 3^8 5^2 7^3이 됨
여기에서 최대공약수 2^2 3^2를 빼면 결국 최소공배수만 남음
